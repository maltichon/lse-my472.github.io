...
p <- p + geom_line(aes(x = date, y = unemp))
p


# Fix axes and add colored rectangles
x_range <- range(unemp$date, na.rm = TRUE)
y_range <- range(unemp$unemp, na.rm = TRUE)
p <- p + scale_x_date(limits = x_range, expand = c(0,0)) 
p

p <- p + geom_rect(data = pres, aes(ymin = 0,ymax = 0.12, xmin=start, 
                               xmax = end, 
                               fill=party), alpha=0.2)
p
# Adding president names

p<-p+geom_text(data=pres, aes(x=start, y=y_range[2]), label=name, angle=90)
p

# Adding lines to separate presidencies

p <- p+geom_vline(data=pres, aes(xintercept = as.numeric(start)),linetype=4)
p
