...
```{r}
current_dir <- "/Users/estelleyeung/Documents/LSE/MT/MY472/GitHub/lse-my472.github.io/week07/"
uol <- read_file(paste(current_dir, "uol.txt", sep = ""))
```

```{r regex}
str_extract_all(uol, '([A-Z][A-Z]?)([0-9])([0-9]?[A-Z]?)\\s?[0-9]([A-Z][A-Z])')
```
...
str_replace_all(uol, '([A-Z]{1,2}[0-9]{1,2}[A-Z]?[ ]{0,1})[0-9][A-Z]{2}', '\\1')
```
