...
bribe_nodes <- html_nodes(bribe_html, css = ".department.clearfix > .paid-amount > span")
...
```{r}
transaction <- html_nodes(bribe_html, css = ".transaction > a") %>% 
  html_text

transaction
```
body > main > div > div > div > div.col-md-8 > div.report-listing.ref-module-paid-bribe > article > section:nth-child(10) > h3 > a
...
scrape_bribe <- function(url){
  ...
  # note that here the c_html is in the front the the operator 
  # but in the latter columns, c_html is in the html_nodes function
  amount <- c_html %>% 
    html_nodes(".department.clearfix > .paid-amount > span") %>%
    # extract it as text
    html_text %>% 
    stri_replace_first_fixed("Paid INR ", "") %>%
...
ggplot(data_all, aes(x = data_all$amount)) + 
  geom_histogram() + 
  scale_x_log10()
